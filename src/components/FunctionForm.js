...
const functionParams = {
  'Add Airplane': [
    ...
    { name: 'maintenanced', type: 'select', options: ['Yes', 'No', 'n/a'], required: true },
    { name: 'model', type: 'text', required: true },
    { name: 'neo', type: 'select', options: ['Yes', 'No', 'n/a'], required: true }
  ],
  'Add Airport': [
  ...
  'Add Person': [
    ...
    { name: 'miles', type: 'text', required: true },
    { name: 'funds', type: 'text', required: true }
  ],
  ...
  'Offer Flight': [
    ...
    { name: 'support_airline', type: 'text', required: true },
    { name: 'support_tail', type: 'text', required: true },
  ...
  'Simulation Cycle': [{name:'Run?', type:'boolean', retuired: true}]
};
...
function FunctionForm({ category, functionName }) {
  ...
  useEffect(() => {
    ...
    const fetchViewData = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:5000/api/view/${functionName}`);
  ...
  const handleSubmit = async (event) => {
    ...
    if (category === 'functions') {
      try {
        ...
        await axios.post(`http://127.0.0.1:5000/api/proc/${procName}`, params);
        setSuccess(true);
        setFormData({});
      } catch (err) {
