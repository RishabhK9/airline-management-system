...
export function FlightTakeoff() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await executeStoredProcedure("flight_takeoff", [formData.flightID])

    if (response.success) {
      setResult({
        success: true,
        message: `Flight ${formData.flightID} has taken off successfully`,
      })
      ...
      const updatedFlights = await executeQuery("SELECT flightID FROM flight WHERE airplane_status = 'on_ground'")
      ...
      setResult({
        success: false,
        message: response.error || "Failed to takeoff flight",
      })
    }
  }
