...
export function PassengersDisembark() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await executeStoredProcedure("passengers_disembark", [formData.flightID])

    if (response.success) {
      setResult({
        success: true,
        message: `Passengers have disembarked from flight ${formData.flightID} successfully`,
      })
    } else {
      setResult({
        success: false,
        message: response.error || "Failed to disembark passengers",
      })
    }
  }
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <div className="flex justify-end space-x-4 mt-6">
            ...
            <Button type="submit">Disembark</Button>
          </div>
