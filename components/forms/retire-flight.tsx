...
export function RetireFlight() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await executeStoredProcedure("retire_flight", [formData.flightID])

    if (response.success) {
      setResult({
        success: true,
        message: `Flight ${formData.flightID} has been retired successfully`,
      })
      // Reset form
      setFormData({
        flightID: "",
      ...
      setResult({
        success: false,
        message: response.error || "Failed to retire flight",
      })
    }
  }
