...
export function PeopleInAir() {
  ...
  const fetchData = async () => {
    setLoading(true)
    const response = await executeView("people_in_the_air")
    ...
    <Card>
      <CardContent className="pt-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-4 text-foreground/70">No people currently in the air</div>
        ) : (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Num Airplanes</TableHead>
              ...
              <TableBody>
                {data.map((row, index) => (
                  <TableRow key={index} className="even:bg-muted/50">
                    ...
                    <TableCell>{row.num_airplanes}</TableCell>
